...
class SequencePredictor(nn.Module):
    ...
    def __init__(self, cfg, dim_in):
        ...
        self.seq_decoder = BahdanauAttnDecoderRNN(
            256, cfg.SEQUENCE.NUM_CHAR, cfg.SEQUENCE.NUM_CHAR, n_layers=1, dropout_p=0.1)
        # self.criterion_seq_decoder = nn.NLLLoss(ignore_index = -1, reduce=False)
        self.criterion_seq_decoder = nn.NLLLoss(
            ignore_index=-1, reduction='none')
        self.rescale = nn.Upsample(
            size=(16, 64), mode='bilinear', align_corners=False)
    ...
    def forward(self, x, decoder_targets=None, word_targets=None, use_beam_search=False):
        ...
        seq_decoder_input_reshape = seq_decoder_input_loc.view(seq_decoder_input_loc.size(
            0), seq_decoder_input_loc.size(1), -1).transpose(0, 2).transpose(1, 2)
        if self.training:
            bos_onehot = np.zeros(
            ...
            if use_beam_search:
                for batch_index in range(seq_decoder_input_reshape.size(1)):
                    ...
                    top_seqs = self.beam_search(seq_decoder_input_reshape[
                                                :, batch_index:batch_index + 1, :], decoder_hidden, beam_size=6, max_len=self.cfg.SEQUENCE.MAX_LENGTH)
                    top_seq = top_seqs[0]
    ...
    def beam_search_step(self, encoder_context, top_seqs, k):
        ...
        for seq in top_seqs:
            ...
            for i in range(k):
                ...
                rs_seq = seq + \
                    [(character_index.item() + 1, character_score.item(),
                      char_score, [decoder_hidden])]
                done = (character_index.item() + 1 ==
                        self.cfg.SEQUENCE.NUM_CHAR - 1)
    ...
    def beam_search(self, encoder_context, decoder_hidden, beam_size=6, max_len=32):
        ...
        top_seqs = [
            [(self.cfg.SEQUENCE.BOS_TOKEN, 1.0, char_score, [decoder_hidden])]]
        # loop
...
class Attn(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        H = hidden.repeat(max_len, 1, 1).transpose(
            0, 1)  # (B, H*W, hidden_size)
...
class BahdanauAttnDecoderRNN(nn.Module):
    ...
    def forward(self, word_input, last_hidden, encoder_outputs):
        ...
        word_embedded_onehot = self.embedding(word_input).view(
            1, word_input.size(0), -1)  # (1,B,embed_size)
        word_embedded = self.word_linear(
            word_embedded_onehot)  # (1, B, hidden_size)
